...
def test_cloud_data_lake():
    ...
    sync_res = lake.sync_trades_to_lakehouse([{"trade_id": "T1", "ticker": "NVDA", "pnl": 500.0}])
    assert sync_res["status"] == "SUCCESS"
    assert sync_res["synced_trades"] == 1

    stream_res = lake.stream_live_portfolio_snapshot(total_equity=105000.0, cash=40000.0, open_positions=3)
...
def test_order_routing_vwap_twap():
    ...
    twap = generate_twap_order_schedule("AAPL", total_shares=6000, current_price=220.0, num_slices=6)
