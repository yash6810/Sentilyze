...
def test_clean_headline_data_valid_tickers_no_cache(temp_data_dirs, mocker):
    ...
    input_data = pd.DataFrame(
        {"date": ["2023-01-01"], "headline": ["Title 1"], "stock": ["AAPL"]}
    )
    input_data.to_csv(input_path, index=False)

    mock_ticker_instance = mocker.patch("yfinance.Ticker")
    mock_ticker_instance.return_value.history.return_value = pd.DataFrame(
        {"Close": [100]}
    )
...
def test_clean_headline_data_loads_from_cache(temp_data_dirs, mocker):
    input_path = os.path.join(temp_data_dirs["raw"], "headlines.csv")
    output_path = os.path.join(temp_data_dirs["raw"], "cleaned_headlines.csv")
    valid_tickers_cache_path = os.path.join(
        temp_data_dirs["processed"], "valid_tickers.json"
    )
...
def test_preprocess_data_orchestrates_correctly(mocker):
    ...
    mock_create_technical_indicators = mocker.patch(
        "src.preprocessing.create_technical_indicators"
    )
    mock_aggregate_sentiment_scores = mocker.patch(
        "src.preprocessing.aggregate_sentiment_scores"
    )
    ...
    mock_get_news.assert_called_once_with(
        ticker, os.environ.get("NEWS_API_KEY"), cache_duration_hours=24
    )
    mock_get_price_history.assert_called_once_with(
