...
def generate_audio_script(signals_list: List[Dict[str, Any]], total_equity: float = 100000.0) -> str:
    ...
    if buys:
        ...
        buy_names = ", ".join([f"{s['ticker']} with {float(s.get('confidence', 0.5))*100:.0f} percent confidence" for s in top_buys])
